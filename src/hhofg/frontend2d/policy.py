...
def _label_set(value: Any, default: Iterable[str] = ()) -> set[str]:
    if value is None:
        return {_norm_label(v) for v in default if _norm_label(v)}
    if isinstance(value, str):
        values = value.split(",")
    else:
        values = list(value)
    return {_norm_label(str(v)) for v in values if _norm_label(str(v))}
...
def policy_from_config(config: dict[str, Any] | None) -> FunctionalGraphPolicy:
    cfg = (config or {}).get("policy", {}) if isinstance(config, dict) else {}
    cabinet_output = bool(cfg.get("cabinet_output", True))
    aggregate_output_objects = _label_set(
        cfg.get("aggregate_output_objects"),
        {"cabinet"} if cabinet_output else set(),
    )
    return FunctionalGraphPolicy(
        suppress_objects=_label_set(cfg.get("suppress_objects"), {"stove"}),
        hint_only_objects=_label_set(cfg.get("hint_only_objects"), {"cabinet"}),
        enable_cabinet_aggregation=bool(cfg.get("enable_cabinet_aggregation", True)),
        aggregate_output_objects=aggregate_output_objects,
        aggregate_candidate_objects=_label_set(cfg.get("aggregate_candidate_objects"), {"cabinet"}),
        drop_lid_nodes=bool(cfg.get("drop_lid_nodes", False)),
        drop_lid_when_overlaps_cap=bool(cfg.get("drop_lid_when_overlaps_cap", False)),
        cap_lid_overlap_iou_thr=float(cfg.get("cap_lid_overlap_iou_thr", 0.9)),
        remote_edge_block_labels=_label_set(cfg.get("remote_edge_block_labels"), set()),
        temp_final_local_edge_invariant=bool(cfg.get("final_local_edge_invariant", False)),
        temp_final_drop_labels=_label_set(cfg.get("final_drop_labels"), set()),
        temp_drop_border_u_nodes=bool(cfg.get("drop_border_u_nodes", False)),
        temp_keep_single_frame_cup_handle=bool(cfg.get("keep_single_frame_cup_handle", False)),
        temp_limit_door_drawer_units=bool(cfg.get("limit_door_drawer_units", False)),
        temp_stage1_min_projected_iou=float(cfg.get("stage1_min_projected_iou", 0.0)),
        cabinet_scene_prior_labels=_label_set(cfg.get("cabinet_scene_prior_labels"), {"cabinet"}),
        cabinet_seed_carriers=_label_set(cfg.get("cabinet_seed_carriers"), {"door", "drawer"}),
        cabinet_unit_labels=_label_set(cfg.get("cabinet_unit_labels"), {"handle", "knob"}),
    )
